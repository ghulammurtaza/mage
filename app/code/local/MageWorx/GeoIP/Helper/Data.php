...
class MageWorx_GeoIP_Helper_Data extends Mage_Core_Helper_Abstract
{
	...
	public function getFlagPath($name = null)
	{
		...
		$filePath = Mage::getSingleton('core/design_package')->getSkinBaseDir().DS.'images'.DS.'flags'.DS.$flagName;
	...
	public function getCurrency($country)
	{
		...
		if ($curBase !== false && count($curBase)) {
			...
			foreach ($curBase as $value) {
				$data = explode(';', $value);
				$curVal = trim($data[1]);
				if ($this->_prepareCountry($data[0]) == $this->_prepareCountry($country)) {
					if (strstr($curVal, ',')) {
